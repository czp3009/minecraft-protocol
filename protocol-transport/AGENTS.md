...
belong in `commonTest`; use Ktor's platform capability report to avoid non-Node JS/Wasm runtimes, which do not expose
TCP, while retaining the same common test on JVM, Android Host, Native, JS/Node, and Wasm/Node. Do not duplicate test
names in Gradle filters. Platform test source sets are for platform-specific oracles or `actual` test support, not
duplicate public-behavior test entries.
