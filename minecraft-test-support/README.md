...
Remote handles expose suspend operations for endpoints, status, logs, commands, events, codec reports, and idempotent
non-blocking close requests. `useRemote` provides structured cleanup. Official paths, process objects, and host file
paths never cross the protocol. World-I/O tests transfer snapshot contents through RPC and work in a local temporary
sandbox owned by the test process.
