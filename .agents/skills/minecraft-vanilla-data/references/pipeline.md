...
- `analyzeOfficialMinecraftPackets`: registered packet classes, ordered fields and codec member references for KSP;
...
- `prepareOfficialMinecraftData`: the aggregate over these producers.
...
- `:protocol-configuration-vanilla:generateVanillaRegistryDataSource` renders encoded registry and block-state payload
  source
  from registry and block reports.
- `:protocol-configuration-vanilla:generateVanillaConfigurationPacketPayloadSource` renders encoded Configuration packet
  payload source from target and Configuration analysis.
- `:datapack-vanilla:generateVanillaDataPackSources` renders a small manifest and one independently loaded
...
`VanillaConfigurationSnapshot` packet decoding, `VanillaConfigurationData` branch selection,
`ConfigurationData`/`ResolvedConfigurationData`, `DataPackConfigurationSnapshot`/`ClientRegistryView`, and
dimension-layout
...
required core at its official lowest-priority position, and return the complete public stack. It must retain persisted
