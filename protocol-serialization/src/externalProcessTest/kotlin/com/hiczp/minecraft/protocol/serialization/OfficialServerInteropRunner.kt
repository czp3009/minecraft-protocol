 ...
 *
 * Process management remains test-only. Network framing uses the portable
 * production transport so the same black-box scenario can run on every host
 * target that can launch the external Java process.
 */
internal object OfficialServerInteropRunner {
    suspend fun run(
        ...
        environment.startOfficialMinecraftServer(
            ...
            try {
                ...
                writeReport(report, server.serverArtifact.sha256, login)
                check(server.stop() == 0) {
    ...
    ): T = SelectorManager(Dispatchers.Default).use { selector ->
        val transport = MinecraftTransport(
            aSocket(selector).tcp().connect("127.0.0.1", port),
        )
        try {
            block(FramedConnection(transport))
        } finally {
            transport.close()
        }
    }
    ...
    private class FramedConnection(
        private val transport: MinecraftTransport,
    ) {
            ...
            val packet = try {
                ...
                throw IllegalStateException(
                    "Could not decode $state clientbound packet " +
                            "0x${packetId.toString(16)} payload=${payload.toHexString()}",
                    failure,
                )
            }
