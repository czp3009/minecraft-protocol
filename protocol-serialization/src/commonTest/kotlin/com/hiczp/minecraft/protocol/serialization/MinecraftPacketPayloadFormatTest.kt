...
class MinecraftPacketPayloadFormatTest {
    @Test
    fun `stream APIs are canonical and honor the caller payload boundary`() {
        ...
        assertEquals(
            primitiveValue,
            MinecraftPacketPayloadFormat.decodeFromSource<PrimitiveValue>(
                source,
                streamed.size,
            ),
        )
    ...
    fun `companion is the default format and factory creates configured instances`() {
        assertSame(MinecraftPacketPayloadFormat, MinecraftPacketPayloadFormat.Default)
        assertEquals(
            MinecraftPacketPayloadFormatConfiguration(),
            MinecraftPacketPayloadFormat.minecraftPacketPayloadFormatConfiguration
        )
        ...
        val configured = MinecraftPacketPayloadFormat(minecraftPacketPayloadFormatConfiguration)

        assertTrue(configured !== MinecraftPacketPayloadFormat.Default)
        assertEquals(minecraftPacketPayloadFormatConfiguration, configured.minecraftPacketPayloadFormatConfiguration)
    }
    ...
    fun `var numbers match protocol golden vectors`() {
        ...
        for ((value, hex) in varInts) {
            assertContentEquals(
                hex.hexToByteArray(),
                MinecraftPacketPayloadFormat.encodeToByteArray(VarIntValue(value)),
            )
            assertEquals(
                VarIntValue(value),
                MinecraftPacketPayloadFormat.decodeFromByteArray<VarIntValue>(hex.hexToByteArray()),
            )
        }
        ...
        for ((value, hex) in varLongs) {
            assertContentEquals(
                hex.hexToByteArray(),
                MinecraftPacketPayloadFormat.encodeToByteArray(VarLongValue(value)),
            )
            assertEquals(
                VarLongValue(value),
                MinecraftPacketPayloadFormat.decodeFromByteArray<VarLongValue>(hex.hexToByteArray()),
            )
        }
    }
    ...
    fun `var numbers round trip deterministic boundary-heavy samples`() {
        ...
        intValues.forEach { value ->
            ...
            assertEquals(
                VarIntValue(value),
                MinecraftPacketPayloadFormat.decodeFromByteArray<VarIntValue>(
                    encoded,
                ),
            )
        }
        ...
        longValues.forEach { value ->
            ...
            assertEquals(
                VarLongValue(value),
                MinecraftPacketPayloadFormat.decodeFromByteArray<VarLongValue>(
                    encoded,
                ),
            )
        }
    ...
    fun `fixed primitives are big endian and unsigned hints are honored`() {
        ...
        assertContentEquals(
            expected,
            MinecraftPacketPayloadFormat.encodeToByteArray(primitiveValue),
        )
        assertEquals(
            expected = primitiveValue,
            actual = MinecraftPacketPayloadFormat.decodeFromByteArray<PrimitiveValue>(expected),
        )
    }
    ...
    fun `strings enforce UTF-16 and UTF-8 limits`() {
        ...
        assertContentEquals(
            ...
            MinecraftPacketPayloadFormat.encodeToByteArray(limitedString),
        )
        assertEquals(
            limitedString,
            MinecraftPacketPayloadFormat.decodeFromByteArray<LimitedString>(
                "04c3a9c3a9".hexToByteArray(),
            ),
        )
    ...
    fun `collections maps nullable and byte shapes round trip`() {
        ...
        val encoded = MinecraftPacketPayloadFormat.encodeToByteArray(collectionValue)
        assertEquals(
            collectionValue,
            MinecraftPacketPayloadFormat.decodeFromByteArray<CollectionValue>(encoded),
        )
        ...
        assertContentEquals(
            byteEncoding,
            MinecraftPacketPayloadFormat.encodeToByteArray(byteShapes),
        )
        assertEquals(
            byteShapes,
            MinecraftPacketPayloadFormat.decodeFromByteArray<ByteShapes>(byteEncoding),
        )
    }
    ...
    fun `byte arrays and general collections have no shared policy budget`() {
        ...
        assertEquals(
            bytesValue,
            MinecraftPacketPayloadFormat.decodeFromByteArray<BytesValue>(
                MinecraftPacketPayloadFormat.encodeToByteArray(bytesValue),
            ),
        )
        assertEquals(
            values,
            MinecraftPacketPayloadFormat.decodeFromByteArray<IntListValue>(
                MinecraftPacketPayloadFormat.encodeToByteArray(values),
            ),
        )
    }
    ...
    fun `registry context and fixed byte widths reject every invalid boundary`() {
        assertFailsWith<IllegalArgumentException> {
            PacketCodecContext(
                ...
                registrySizeOverrides = mapOf(
                    PacketCodecContext.BIOME_REGISTRY to 0,
                ),
            )
        }
        ...
        assertFailsWith<MinecraftSerializationException> {
            MinecraftPacketPayloadFormat.decodeFromByteArray<FixedBytes>(
                byteArrayOf(1),
            )
        }
    }
    ...
    fun `network NBT has exact unnamed encoding`() {
        ...
        val encoded = MinecraftPacketPayloadFormat.encodeToByteArray(nbtValue)
        assertEquals(
            nbtValue,
            MinecraftPacketPayloadFormat.decodeFromByteArray<NbtValue>(encoded),
        )
    ...
    fun `network NBT strings use Java modified UTF`() {
        ...
        assertContentEquals(
            expected,
            MinecraftPacketPayloadFormat.encodeToByteArray(nbtValue),
        )
        assertEquals(
            expected = nbtValue,
            actual = MinecraftPacketPayloadFormat.decodeFromByteArray<NbtValue>(
                expected,
            ),
        )
    }
    ...
    fun `network NBT failures retain their serialization exception type`() {
        val failure = assertFailsWith<NbtDecodingException> {
            MinecraftPacketPayloadFormat.decodeFromByteArray<NbtValue>(
                byteArrayOf(99),
            )
        }
    ...
    fun `length-prefixed nullable NBT uses TAG End rather than boolean`() {
        ...
        assertEquals(
            absent,
            MinecraftPacketPayloadFormat.decodeFromByteArray<LengthPrefixedOptionalNbt>(
                "0100".hexToByteArray(),
            ),
        )
        ...
        assertEquals(
            expected = present,
            actual = MinecraftPacketPayloadFormat.decodeFromByteArray<LengthPrefixedOptionalNbt>(
                expected,
            ),
        )
        assertFailsWith<MinecraftSerializationException> {
    ...
    fun `low precision vectors match vanilla golden bytes`() {
        ...
        assertContentEquals(
            "00".hexToByteArray(),
            MinecraftPacketPayloadFormat.encodeToByteArray(zero),
        )

        val unitX = LowPrecisionVectorValue(Vector3d(1.0, 0.0, 0.0))
        assertContentEquals(
            "f1ff7ffeffff".hexToByteArray(),
            MinecraftPacketPayloadFormat.encodeToByteArray(unitX),
        )
        val unitDecoded = MinecraftPacketPayloadFormat.decodeFromByteArray<LowPrecisionVectorValue>(
            "f1ff7ffeffff".hexToByteArray(),
        )
        ...
        assertEquals(
            continuation,
            MinecraftPacketPayloadFormat.decodeFromByteArray<LowPrecisionVectorValue>(
                "f4ff7ffeffff01".hexToByteArray(),
            ),
        )
    ...
    fun `low precision vectors sanitize and validate their compact representation`() {
        ...
        val decoded = MinecraftPacketPayloadFormat.decodeFromByteArray<LowPrecisionVectorValue>(
            "f7ff7ffe0003ffffffff0f".hexToByteArray(),
        )
    ...
    fun `malformed lengths varints booleans and trailing bytes are rejected`() {
        ...
        assertFailsWith<MinecraftSerializationException> {
            MinecraftPacketPayloadFormat.decodeFromByteArray<NullableValue>(
                "02".hexToByteArray(),
            )
        }
        assertFailsWith<MinecraftSerializationException> {
        ...
        val strict = MinecraftPacketPayloadFormat(
            MinecraftPacketPayloadFormatConfiguration(rejectNonMinimalVarNumbers = true),
        )
        assertFailsWith<MinecraftSerializationException> {
        ...
        val permissive = MinecraftPacketPayloadFormat(
            MinecraftPacketPayloadFormatConfiguration(strictBooleans = false),
        )
        assertEquals(
            NullableValue(7),
