...
class PlayerStatisticsStore(
    ...
    fun readJson(playerUuid: String): JsonElement? =
        utf8JsonFileStore.readJsonElementOrNull(minecraftWorldPaths.statistics(playerUuid))

    fun <T> read(playerUuid: String, deserializationStrategy: DeserializationStrategy<T>): T? =
        utf8JsonFileStore.readJsonOrNull(minecraftWorldPaths.statistics(playerUuid), deserializationStrategy)
    ...
    fun <T> read(playerUuid: String, block: (BufferedSource) -> T): T? =
        utf8JsonFileStore.readJsonOrNull(minecraftWorldPaths.statistics(playerUuid), block)

    fun writeJson(playerUuid: String, jsonElement: JsonElement) =
        utf8JsonFileStore.writeJsonElement(minecraftWorldPaths.statistics(playerUuid), jsonElement)

    fun <T> write(
        playerUuid: String,
        value: T,
        serializationStrategy: SerializationStrategy<T>,
    ) = utf8JsonFileStore.writeJson(minecraftWorldPaths.statistics(playerUuid), value, serializationStrategy)
    ...
    fun write(playerUuid: String, block: (BufferedSink) -> Unit) =
        utf8JsonFileStore.writeJson(minecraftWorldPaths.statistics(playerUuid), block)
}
