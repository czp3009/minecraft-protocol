...
`server-properties-compatibility.json` binds every property and default generated by the matching official server to a
reviewed scope, library support mechanism, and consuming-server responsibility. The Gradle compatibility check compares
it with a fresh build-local `--initSettings` result so additions and semantic defaults cannot pass unnoticed.
