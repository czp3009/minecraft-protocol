...
internal val buildLogicJson = Json {
...
internal fun Path.writeJson(
    ...
    atomicWriteText("${buildLogicJson.encodeToString(document)}\n")
}
...
internal object DownloadHttp {
    ...
    private val logger = Logging.getLogger(DownloadHttp::class.java)
    ...
    suspend fun getJson(
        ...
        getBytes(url, connectTimeout, offline) { bytes ->
            content = buildLogicJson.decodeFromString<JsonObject>(bytes.decodeToString())
        }
        return checkNotNull(content)
    }
...
internal data class OfficialMinecraftTarget(
...
internal data class OfficialMinecraftTargetReport(
    val officialMinecraftTarget: OfficialMinecraftTarget,
)

internal fun OfficialMinecraftTarget.toOfficialMinecraftTargetReportJson(): JsonObject = buildJsonObject {
...
internal fun Path.readOfficialMinecraftTargetReport(): OfficialMinecraftTargetReport {
    ...
    val report = buildLogicJson.decodeFromString<JsonObject>(readText())
...
internal fun Path.readOfficialMinecraftTarget(): OfficialMinecraftTarget {
    check(isRegularFile()) {
        "Official server JAR is missing: $this"
    }
    val version = buildLogicJson.decodeFromString<JsonObject>(readZipEntry("version.json").decodeToString())
    val minecraftVersion = version.getValue("id").jsonPrimitive.content
    return OfficialMinecraftTarget(
